# Getting Started with Diamonds kata in Typescript
...
> ***Reminder***: the command below should be run from [Kata-Diamonds/typescript](.) directory
...
`File` > `Open` > `Kata-Diamonds` > `typescript`
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-Diamonds` / `typescript`
