# Getting Started with Diamonds kata in Python
...
> ***Reminder***: the commands below should be run from [Kata-Diamonds/python](.) directory
...
`File` > `Open` > `Kata-Diamonds` > `python`
...
> ***Reminder***: the command below should be run from [Kata-Diamonds/python](.) directory
...
Choose `Open Folder`, navigate to `Kata-Diamonds` / `python` then click `Select Folder`.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-Diamonds` / `python`
