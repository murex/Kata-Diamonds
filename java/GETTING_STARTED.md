# Getting Started with Diamonds kata in Java
...
`File` > `Open Folder` > `Kata-Diamonds` > `java`
...
`File` > `Open Projects from File System...` > `Kata-Diamonds`
...
3. From the terminal navigate to [Kata-Diamonds/java]() directory
