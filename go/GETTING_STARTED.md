# Getting Started with Diamonds kata in Go
...
> ***Reminder***: the commands below should be run from [Kata-Diamonds/go]() directory
...
> ***Reminder***: the command below should be run from [Kata-Diamonds/go]() directory
...
`File` > `Open` > `Kata-Diamonds` > `go`

Run all the tests in [diamonds_test.go](./src/diamonds/diamonds_test.go).
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-Diamonds` / `go`
