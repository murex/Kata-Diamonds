# Getting Started with Diamonds kata in C++
...
- Create a build directory: ***Kata-Diamonds/cpp/build***. All build-related files are generated under this directory.
...
- Generate the solution file ***Kata-Diamonds.sln*** for **Visual Studio 2019** on Windows,
...
Open Xcode, choose `Open a project or file`, navigate to `Kata-Diamonds` / `cpp` / `build` / `Kata-Diamonds.xcodeproj`
then click `Open`.

Once the project is opened in Xcode, select `Product` / `Scheme` / `Kata-Diamonds-test` to set it as the current active scheme.
